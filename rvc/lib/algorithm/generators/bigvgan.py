...
class UpSample1d(nn.Module):
    ...
    def forward(self, x):
        x = self.ratio * F.conv_transpose1d(
            F.pad(x, (self.pad, self.pad), mode="replicate"),
...
class LowPassFilter1d(nn.Module):
    ...
    def forward(self, x):
        return F.conv1d(
            F.pad(x, (self.pad_left, self.pad_right), mode="replicate"),
            self.filter.expand(x.size(1), -1, -1),
            stride=self.stride,
            groups=x.size(1),
        )
...
class AMPLayer(nn.Module):
    ...
    def remove_weight_norm(self):
        _remove_weight_norm(self.conv1)
        _remove_weight_norm(self.conv2)
...
class SineGen(nn.Module):
    ...
    def forward(self, f0):
        with torch.no_grad():
            f0_buf = torch.zeros(
                f0.shape[0], f0.shape[1], self.dim, dtype=f0.dtype, device=f0.device
            )
            f0_buf[:, :, 0] = f0[:, :, 0]
            ...
            sine_waves = sine_waves * uv + (
                (uv * self.noise_std + (1 - uv) * self.sine_amp / 3)
                * torch.randn_like(sine_waves)
            )
...
class SourceModuleHnNSF(nn.Module):
    ...
    def forward(self, x):
        sine_merge = self.l_tanh(
            self.l_linear(self.l_sin_gen(x).to(dtype=self.l_linear.weight.dtype))
        )
...
class BigVGANGenerator(nn.Module):
    ...
    def forward(self, x, f0, g=None):
        har_source = self.m_source(
            self.f0_upsample(f0[:, None, :]).transpose(-1, -2)
        ).transpose(-1, -2)
        x = self.conv_pre(x)
        if g is not None:
            x += self.cond(g)
